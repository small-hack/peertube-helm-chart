...
version=$(grep "^version:" "charts/peertube/Chart.yaml" | awk '{print $2}')
...
echo "Bumping version for peertube chart from $version to $major.$minor.$patch"
sed -i "s/^version:.*/version: ${major}.${minor}.${patch}/g" "charts/peertube/Chart.yaml"
